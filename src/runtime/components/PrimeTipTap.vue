<script setup>
import { ref, defineProps, computed, watch, defineEmits, onMounted, onBeforeUnmount } from 'vue'
...
const props = defineProps({
  ...
  extensions: {
    ...
    default: [StarterKit, Highlight, TextAlign.configure({ types: ['heading', 'paragraph'] })]
  },
  toolbarVisible: {
    type: Boolean,
    default: true
...
<template>
  <div v-if="editor">
    <Toolbar v-if="toolbarVisible && extensionNames.includes('starterKit')" id="toolbar">
      <template #start>
        <Button
          id="bold"
          size="small"
